...
usage() {
  cat <<'USAGE'
Usage: tools/remote_render/sync_one_slide.sh --slideId slide_id [options]
...
runs npm run render:slide-video on the render machine for the selected slide.
USAGE
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --slideId|--slide-id) SLIDE_ID="${2:?Missing value for --slideId}"; shift 2 ;;
...
if [[ -z "$SLIDE_ID" ]]; then
  echo "Missing required --slideId." >&2
  usage >&2
  exit 2
fi
...
if [[ "$RUN_RENDER" -eq 1 ]]; then
  if [[ "$DRY_RUN" -eq 1 ]]; then
    echo "Dry run: would render slide $SCHOOL/$COURSE/$SESSION/$TOPIC/$SLIDE_ID on $RENDER_USER@$RENDER_HOST"
  else
    ...
    CMD="cd $(quote_arg "$RENDER_REPO") && npm run render:slide-video -- --school $(quote_arg "$SCHOOL") --course $(quote_arg "$COURSE") --session $(quote_arg "$SESSION") --topic $(quote_arg "$TOPIC") --slideId $(quote_arg "$SLIDE_ID")"
